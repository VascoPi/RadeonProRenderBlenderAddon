...
class Grid(pyrpr.Grid):
    ...
    def init_from_3d_array(context, grid_data: np.ndarray):
        ...
        indices = np.nonzero(grid_data)
        grid = pyrpr.Grid.init_from_array_indices(context, x, y, z, grid_data[indices], np.ascontiguousarray(np.transpose(indices).astype('uint32')))
